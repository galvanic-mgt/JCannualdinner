...
(function(){
  ...
  function updateButton(){
    ...
    btn.textContent = isLight ? '?? Dark Mode' : '?ÄÔ∏?Normal Mode';
  }
})();
...
function ensureQR(link){
    ...
    <div class="grid-2" style="gap:16px;align-items:center">
      ...
      <div>
        ...
        <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap">
          <button id="copyVoteLink" class="btn">Ë§áË£Ω?ïÁ•®???</button>
          <a class="btn" href="${link}" target="_blank" rel="noopener">?ãÂ??ïÁ•®??/a>
        </div>
      </div>
    </div>
  `;
  ...
  document.getElementById('copyVoteLink')?.addEventListener('click', async () => {
    try { await navigator.clipboard.writeText(link); alert('Â∑≤Ë?Ë£ΩÊ?Á•®ÈÄ??'); } catch(e){}
  });
}
  ...
  if (!card) {
    ...
    card.innerHTML = `
      <div class="bar" style="justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap">
        <div>
          <strong>?æÂ†¥?±Âà∞ / Landing Page</strong>
          <p class="muted" style="margin-top:4px;font-size:13px">
            ?ôÂÄãÈÄ????QR ?ØÁµ¶?æÂ†¥?ÉÂ??ÖÂ†±?∞Áî®?ÑÔ?
            ?ÉÊ?ÂæåÊ??ãÂ? <code>landing.html?event=??/code>Ôºå‰∏¶??é•?∞ÁõÆ?çÈÄôÂÄãÊ¥ª?ï„Ä?          </p>
        </div>
  ...
  host.innerHTML = `
    <div class="grid-2" style="gap:16px;align-items:center">
      ...
      <div>
        <div class="muted" style="word-break:break-all">${link}</div>
        <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap">
          <button id="copyLandingLink" class="btn">Ë§áË£Ω Landing ???</button>
          <a class="btn" href="${link}" target="_blank" rel="noopener">?ãÂ? Landing ??/a>
        </div>
      </div>
    </div>
  `;
...
function bindLandingButton(){
  ...
  btn.addEventListener('click', (e)=>{
    ...
    if (!eid) { alert('Ë´ãÂ??®Â∑¶?¥ÈÅ∏?á‰??ãÊ¥ª??); return; }
...
function requireMasterPassword(){
  ...
  if (!active?.id) { alert('Ë´ãÂ??ªÂÖ• Master Â∏≥Ë?'); return false; }
  if (active.role !== ROLE_MASTER) { alert('?™Ê? Master ?Ø‰ª•ËÆäÊõ¥‰ΩøÁî®??); return false; }
  const pwd = prompt('Ë´ãËº∏??Master ÂØÜÁ¢º‰ª•ÁπºÁ∫åÔ?')?.trim();
  if (pwd !== (active.password || '')) { alert('ÂØÜÁ¢º‰∏çÊ≠£Á¢?); return false; }
  return true;
}
...
function renderUsersUI(){
  ...
  if (!users.length) {
    ...
    tr.innerHTML = '<td colspan="4" class="muted">Â∞öÊú™Âª∫Á?‰ΩøÁî®?ÖÔ?Ë´ã‰Ωø?®È?Ë®≠Â∏≥?üÁôª?•Ô?administrator / administrator</td>';
  ...
  users.forEach(u=>{
    ...
    tr.innerHTML = `
      ...
      <td>${u.role === ROLE_MASTER ? 'Master' : '?çÂñÆÁÆ°Á?'}</td>
      <td>${(u.events||[]).length ? u.events.join(', ') : '?®ÈÉ®'}</td>
      <td>
        <button class="btn small" data-edit="${u.id}">Á∑®ËºØ</button>
        <button class="btn small danger" data-del="${u.id}">?™Èô§</button>
      </td>`;
    tr.querySelector('[data-edit]')?.addEventListener('click', async ()=>{
      if (!requireMasterPassword()) return;
      const name = prompt('?çÁ®±Ôº?, u.name || '')?.trim();
      if (!name) return;
      const username = prompt('?ªÂÖ•Â∏≥Ë?Ôº?, u.username || '')?.trim();
      if (!username) return;
      const password = prompt('?ªÂÖ•ÂØÜÁ¢ºÔº?, u.password || '')?.trim();
      if (!password) return;
      const role = prompt('ËßíËâ≤Ôºàmaster / rosterÔºâÔ?', u.role || ROLE_ROSTER)?.trim() || ROLE_ROSTER;
      const eventsRaw = prompt('?ÅË®±?ÑÊ¥ª??IDÔºàÁî®?óË??ÜÈ?ÔºåÁ?Á©??®ÈÉ®ÔºâÔ?', (u.events||[]).join(','));
      ...
      try {
        ...
        alert('?°Ê??≤Â?‰ΩøÁî®?ÖÔ?Ë´ãÁ¢∫Ë™çÊ??êÊ?Ë¶èÂ?Ë®≠Â???);
      }
    });
    tr.querySelector('[data-del]')?.addEventListener('click', async ()=>{
      ...
      try {
        ...
        alert('?°Ê??™Èô§‰ΩøÁî®?ÖÔ?Ë´ãÁ¢∫Ë™çÊ??êÊ?Ë¶èÂ?Ë®≠Â???);
      }
    });
    tbody.appendChild(tr);
  });
  ...
  users.forEach(u=>{
    ...
    opt.value = u.id;
    opt.textContent = `${u.name} (${u.role === ROLE_MASTER ? 'Master' : '?çÂñÆ'})`;
...
function bindUsers(){
  document.getElementById('btnAddUser')?.addEventListener('click', async ()=>{
    ...
    if (!name) return;
    const username = prompt('Ë®≠Â??ªÂÖ•Â∏≥Ë?Ôº?)?.trim();
    const password = prompt('Ë®≠Â??ªÂÖ•ÂØÜÁ¢ºÔº?)?.trim();
    if (!username || !password) return;
    const eventsRaw = prompt('?ÅË®±?ÑÊ¥ª??IDÔºàÁî®?óË??ÜÈ?ÔºåÁ?Á©??®ÈÉ®ÔºâÔ?','');
    const events = eventsRaw ? eventsRaw.split(',').map(s=>s.trim()).filter(Boolean) : [];
...
async function renderEventList(){
  ...
  list.forEach(ev=>{
    ...
    item.onclick = async ()=>{
      ...
      if (current && current !== ev.id) {
        const ok = confirm(`?≥Â??áÊ??∞„Ä?{ev.name}?çÊ¥ª?ïÔ?Á¢∫Â??éÔ?`);
        if (!ok) return;
      }
  ...
  ad.innerHTML = `
    <input id="newEventName" placeholder="?∞Â?Ê¥ªÂ??çÁ®±" />
    <input id="newClientName" placeholder="ÂÆ¢Êà∂?çÁ®±" />
    <button id="btnAddEvent" class="btn primary">+ ?∞Ê¥ª??/button>`;
  el.appendChild(ad);
...
function updateRosterCounters(list = []){
  ...
  if (chkEl)   chkEl.textContent   = `Â∑≤Â†±?∞Ô?${checked} ‰∫∫`;
}
...
async function setPeopleWithSync(eid, people){
  setRosterSyncStatus('?¥Êñ∞‰∏≠‚Ä?);
  try {
    await setPeople(eid, people);
    setRosterSyncStatus('Â∑≤Êõ¥??);
  } catch (err) {
    console.error('[roster] sync failed', err);
    setRosterSyncStatus('?¥Êñ∞Â§±Ê?');
    throw err;
  }
}
function makeLogKey(){
...
async function exportAttendanceLog(eid){
  ...
  list.forEach(entry=>{
    ...
    const action = entry?.action === 'cancel' ? '?ñÊ?' : '?∫Â∏≠';
    rows.push([
      ts,
      action,
      entry?.name || '',
...
function renderRow(tr, p, idx, mode){
  if(mode === 'edit'){
    tr.innerHTML = `
      ...
      <td>${rewardText(p)}</td>
      <td>
        <button class="btn small save">?≤Â?</button>
        <button class="btn small cancel">?ñÊ?</button>
      </td>
    `;
    ...
    tr.innerHTML = `
      ...
      <td>
        ...
        ${p.prize ? '<button class="btn small" data-clear-win>Ê∏ÖÈô§ÂæóÁ?</button>' : ''}
        <button class="btn small danger delete">?™Èô§</button>
      </td>
    `;
    tr.querySelector('td input[type="checkbox"]').onchange = async (e)=>{
      const next = e.target.checked;
    ...
    tr.querySelector('[data-clear-win]')?.addEventListener('click', async ()=>{
      if (!p.prize) return;
      const ok = confirm(`Á¢∫Â?Ë¶ÅÁßª?§„Ä?{p.name||''}?çÁ?ÂæóÁ?Á¥Ä?ÑÔ?${p.prize}ÔºâÔ?`);
      if (!ok) return;
    ...
    tr.querySelector('.delete').onclick = async ()=>{
      const ok = confirm(`Á¢∫Â??™Èô§??{p.name||''}?çÔ?`);
      if(!ok) return;
      people.splice(idx, 1);
      await setPeopleWithSync(eid, people);
      await renderRoster();
...
function bindRoster(){
  ...
  document.getElementById('btnDeleteAllRoster')?.addEventListener('click', async ()=>{
    const eid = getCurrentEventId(); if(!eid) return;
    const ok = confirm('Á¢∫Â?Ë¶ÅÊ?Á©∫ÂÖ®?®Â??ÆÔ?Ê≠§Â?‰ΩúÁÑ°Ê≥ïÂæ©?ü„Ä?);
    if(!ok) return;
  ...
  document.getElementById('btnAddManual')?.addEventListener('click', async ()=>{
    const eid = getCurrentEventId(); if(!eid) return;
    const name = prompt('ÂßìÂ?Ôº?)?.trim();
    if (!name) return;
    const dept  = prompt('?®È? / ?èËø∞ÔºàÂèØ?ôÁ©∫ÔºâÔ?')?.trim() || '';
    const phone = prompt('?ªË©±ÔºàÂèØ?ôÁ©∫ÔºâÔ?')?.trim() || '';
    const code  = prompt('‰ª?¢ºÔºàÂèØ?ôÁ©∫ÔºâÔ?')?.trim() || '';
    const table = prompt('?±Ë?ÔºàÂèØ?ôÁ©∫ÔºâÔ?')?.trim() || '';
    const seat  = prompt('Â∫ß‰?ÔºàÂèØ?ôÁ©∫ÔºâÔ?')?.trim() || '';
    const checkedIn = confirm('?ØÂê¶Ê®ôË??∫„ÄåÂá∫Â∏≠„ÄçÔ?');
...
async function renderPrizes(){
  ...
  list.forEach(p=>{
    ...
    tr.innerHTML = `
      ...
      <td>${used}</td>
      <td>
        <button class="btn small" data-edit="${p.id}">Á∑®ËºØ</button>
        <button class="btn small danger" data-del="${p.id}">?™Èô§</button>
      </td>`;
    ...
    tr.querySelector('[data-del]').onclick = async ()=>{
      const go = used>0
        ? confirm(`??{p.name||'Ê≠§Á???}?çÂ∑≤??${used} ‰ΩçÂ??éËÄÖ„ÄÇ\nÁ¢∫Â?Ë¶ÅÂà™?§Â?Ôºü`)
        : confirm(`?™Èô§??{p.name||'Ê≠§Á???}?çÔ?`);
      if(!go) return;
      await removePrize(p.id);
      await renderPrizes();
    };
    tr.querySelector('[data-edit]').onclick = async ()=>{
      const newNo   = prompt('?¥Êñ∞?éÂ?Á∑®Ë?Ôº?, p.no || '')?.trim() || '';
      const newName = prompt('?¥Êñ∞?éÂ??çÁ®±Ôº?, p.name || '')?.trim();
      if (!newName) return;
      const newQuotaRaw = prompt('?¥Êñ∞?çÈ?ÔºàÊï∏Â≠óÔ?Ôº?, String(p.quota || 1))?.trim();
      if (newQuotaRaw === null) return;
      const newQuota = Math.max(0, Number(newQuotaRaw) || 0);
      const quotaChanged = newQuota !== Number(p.quota || 0);
      if (quotaChanged) {
        const ok = confirm(`?çÈ?Â∞áÊîπ??${newQuota}ÔºåÁ¢∫ÂÆöË?‰øÆÊîπ?éÔ?`);
        if (!ok) return;
      }
  ...
  if (wins) {
    ...
    (prizes||[]).forEach(
      p=>(p.winners||[]).forEach(w=>{
        ...
        li.textContent=`${w.name}Ôº?{p.name}Ôºâ`;
        wins.appendChild(li);}));
  }
}
...
async function renderAssets(){
  ...
  if (!photos.length) {
    ...
    p.className = 'muted';
    p.textContent = 'Â∞öÊú™?†ÂÖ•‰ªª‰??∏Á? URL??;
    grid.appendChild(p);
    return;
  }
  ...
  photos.forEach((url, i) => {
    ...
    wrap.innerHTML = `
      ...
      <button class="btn small" type="button" data-delete-photo="${i}">?™Èô§</button>
    `;
...
function bindAssets(){
  ...
  $('saveAssets')?.addEventListener('click', async () => {
    const eid = getCurrentEventId();
    if (!eid) {
      alert('Ë´ãÂ??®Â∑¶?¥ÈÅ∏?á‰??ãÊ¥ª?ïÔ?EventÔºâ„Ä?);
      return;
    }
...
function normalizeGameBooths(raw){
  ...
  return list
    ...
    .map((booth, i) => ({
      ...
      name: String(booth.name || `?äÊà≤?§‰? ${i + 1} (Game Booth ${i + 1})`),
  ...
  if (linkHost) {
    linkHost.innerHTML = `
      <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
        <strong>Ë®™ÂÆ¢?ÅÈù¢ (Visitor page):</strong>
  ...
  booths.forEach((booth, index) => {
    ...
    card.innerHTML = `
      <div class="game-booth-head">
        <label>
          <span>?§‰??çÁ®± (Booth name)</span>
          <input data-game-booth-name value="">
        </label>
        <button class="btn small danger" type="button" data-game-booth-delete>?™Èô§ (Delete)</button>
      </div>
...
function collectGameBoothCards(){
  return Array.from(document.querySelectorAll('#gameBoothList .game-booth-card')).map((card, index) => ({
    ...
    name: card.querySelector('[data-game-booth-name]')?.value.trim() || `?äÊà≤?§‰? ${index + 1} (Game Booth ${index + 1})`,
    active: true
  }));
}
...
function bindGameBooths(){
  document.getElementById('btnAddGameBooth')?.addEventListener('click', async () => {
    ...
    booths.push({ id: `booth_${stamp}`, name: `?äÊà≤?§‰? ${booths.length + 1} (Game Booth ${booths.length + 1})`, active: true });
...
async function renderPolls(){
  ...
  for (const [pid, p] of Object.entries(polls)) {
    ...
    li.innerHTML = `
      <strong>${p.question || p.q || '(?™ÂëΩ??'}</strong>
      <small>(??${total} Á•?</small>
      ...
      <div class="bar" style="gap:6px;margin-top:6px;flex-wrap:wrap">
        ...
        <a class="btn" data-act="public" href="${publicUrl}" target="_blank" rel="noopener">?¨Áúæ?´Èù¢</a>
        <button class="btn" data-act="use">‰ΩøÁî®Ê≠§Â?È°?/button>
      </div>
    `;
    ...
    li.querySelector('[data-act="qr"]').onclick = () => {
      ...
      host.innerHTML = `
        <div class="grid-2" style="gap:16px;align-items:center">
          ...
          <div>
            ...
            <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap">
              <button id="copyVoteLink" class="btn">Ë§áË£Ω?ïÁ•®???</button>
              <a class="btn" href="${voteUrl}" target="_blank" rel="noopener">?ãÂ??ïÁ•®??/a>
            </div>
          </div>
        </div>
      `;
      ...
      if (copyBtn) copyBtn.addEventListener('click', async () => {
        try { await navigator.clipboard.writeText(voteUrl); alert('Â∑≤Ë?Ë£ΩÊ?Á•®ÈÄ??'); } catch(e) {}
      });
    };
...
function bindPolls(){
  ...
  btn.addEventListener('click', async ()=>{
    ...
    await renderPolls();
    await renderPollManager(); // keep ?ïÁ•®ÁÆ°Á? list in sync
...
async function bindPollPicker(){
  ...
  if (entries.length === 0) {
    const opt = document.createElement('option');
    opt.value = '';
    opt.textContent = 'ÔºàÂ??™Âª∫Á´ãÊ?Á•®Ô?';
    sel.appendChild(opt);
  } else {
    for (const [pid, p] of entries) {
      const opt = document.createElement('option');
      opt.value = pid; // use RTDB key as id
      const title = (p && (p.question || p.q)) ? (p.question || p.q) : '(?™ÂëΩ??';
...
function bindPollComposer(){
  ...
  btnCreate?.addEventListener('click', async ()=>{
    ...
    if (!eid || !q || !opts.length) { alert('Ë´ãËº∏?•Â?È°åË??≥Â?‰∏Ä?ãÈÅ∏??); return; }
    const poll = {
      id: makeId('p'),
      question: q,
...
function bindPrizeActions(){
  ...
  document.getElementById('btnImportPrizeCSV')?.addEventListener('click', ()=>{
    ...
    if(!f?.files?.[0]){ alert('Ë´ãÂ??∏Ê? CSV Ê™îÊ?'); return; }
  ...
  document.getElementById('btnDeleteAllPrizes')?.addEventListener('click', async ()=>{
    const ok = confirm('Á¢∫Â?Ë¶ÅÂà™?§Ê??âÁ??ÅË?ÂæóÁ?Á¥Ä?ÑÔ?Ê≠§Â?‰ΩúÁÑ°Ê≥ïÂæ©?ü„Ä?);
    if(!ok) return;
...
export async function renderPollManager() {
  ...
  if (!entries.length) {
    list.innerHTML = '<li class="muted">Â∞öÊú™Âª∫Á?‰ªª‰??ïÁ•®?èÈ?</li>';
    return;
  }
  ...
  for (const [pid, p] of entries) {
    ...
    li.innerHTML = `
      <div style="display:flex;flex-direction:column;gap:4px">
        <input class="poll-question" value="${poll.question || ''}" placeholder="Ëº∏ÂÖ•?èÈ?..." />
        <div class="poll-options"></div>
        <div class="bar" style="gap:6px;margin-top:4px;flex-wrap:wrap">
          <button class="btn" data-act="save">?íæ ?≤Â?</button>
          <button class="btn" data-act="delete" style="background:#b71c1c;color:white">?™Èô§</button>
        </div>
      </div>
    `;
    ...
    optWrap.innerHTML = options
      .map((o, i) => {
        ...
        <div class="bar poll-opt-row" style="gap:4px;align-items:center" data-id="${optId}">
          <input class="poll-opt" data-field="text" value="${text}" placeholder="?∏È? ${i + 1}" />
          <input class="poll-opt-img" data-field="img" value="${img}" placeholder="?ñÁ? URLÔºàÂèØ?∏Ô?" style="flex:1; min-width:160px" />
          ${img ? `<img src="${img}" style="width:48px;height:48px;object-fit:cover;border-radius:8px;border:1px solid rgba(255,255,255,.12)" alt="">` : ''}
          ${isLast ? '<button class="btn btn-small" data-act="addopt">Ôº?/button>' : ''}
        </div>`;
      })
      .join('') || `
        <div class="bar poll-opt-row" style="gap:4px;align-items:center" data-id="o0">
          <input class="poll-opt" data-field="text" value="" placeholder="?∏È? 1" />
          <input class="poll-opt-img" data-field="img" value="" placeholder="?ñÁ? URLÔºàÂèØ?∏Ô?" style="flex:1; min-width:160px" />
          <button class="btn btn-small" data-act="addopt">Ôº?/button>
        </div>`;
    ...
    li.addEventListener('click', async (e) => {
      ...
      if (act === 'save') {
        ...
        if (!question || !opts.length) return alert('Ë´ãËº∏?•Â?È°åË??≥Â?‰∏Ä?ãÈÅ∏??);
      ...
      if (act === 'delete') {
        if (!confirm('Á¢∫Â??™Èô§Ê≠§Â?È°åÔ?')) return;
      ...
      if (act === 'addopt') {
        ...
        div.innerHTML = `
          <input class="poll-opt" data-field="text" value="" placeholder="?∏È? ${n}" />
          <input class="poll-opt-img" data-field="img" value="" placeholder="?ñÁ? URLÔºàÂèØ?∏Ô?" style="flex:1; min-width:160px" />
          <button class="btn btn-small" data-act="addopt">Ôº?/button>
        `;
        bar.appendChild(div);
...
export async function bootCMS(){
  ...
  // core binders (don?ôt crash if any is undefined)
  ...
  if (typeof renderAll === 'function') {
    ...
    // legacy compatibility: render per-tab if needed (won?ôt throw)
