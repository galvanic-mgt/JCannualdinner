...
export async function renderRerollLog(){
  ...
  let html = '<table><thead><tr><th>?‚é?</th><th>?Žé?</th><th>?Ÿå?ä¸?/th><th>?¹ç‚º</th></tr></thead><tbody>';
  for(const row of (list||[])){
    const t = row.time ? new Date(row.time).toLocaleString() : '';
    const ori = row.replaced ? `${row.replaced.name||''}ï¼?{row.replaced.dept||''}ï¼‰` : '';
    const rep = row.replacement ? `${row.replacement.name||''}ï¼?{row.replacement.dept||''}ï¼‰` : '<span style="opacity:.6">??/span>';
...
export async function undoLastDraw(){
  ...
  if (!batch.length) throw new Error('?®å?æ²’æ??¯ä»¥å¾©å??„æŠ½??);
