...
function mapHeaderIndex(headers, info){
  ...
  return {
    code:     hp(['code','ç¥¨è?','ä»?¢¼']),
    phone:    hp(['phone','mobile','tel','?»è©±', labelPhone]),
    name:     hp(['name','å§“å?']),
    dept:     hp(['department','dept','?è¿°','èªªæ?','?¨é?','description', labelDept]),
    table:    hp(['table','??,'æ¡?,'æ¡Œè?','?±è?']),
    seat:     hp(['seat','åº§ä?','åº§è?']),
    present:  hp(['present','checkedin','checkin','?ºå¸­','?°å ´','?ºå¸­?‡å¦','??,'?¨å ´'])
  };
}
