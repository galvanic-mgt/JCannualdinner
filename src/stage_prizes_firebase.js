...
export async function updatePrize(patch = {}) {
  const eid = getCurrentEventId();
  if (!eid) throw new Error('å°šæœª?¸æ?æ´»å?');
  if (!patch.id) throw new Error('ç¼ºå??Žé? ID');

  const prizes = (await getPrizes(eid)) || [];
  const idx = prizes.findIndex(p => p?.id === patch.id);
  if (idx < 0) throw new Error('?¾ä??°ç??…ï?' + patch.id);
...
export async function removePrize(prizeId) {
  const eid = getCurrentEventId();
  if (!eid) throw new Error('å°šæœª?¸æ?æ´»å?');
  if (!prizeId) throw new Error('ç¼ºå??Žé? ID');
...
export async function setCurrentPrize(prizeId) {
  ...
  if (pid) {
    ...
    if (!exists) throw new Error(`?¾ä??°ç??…ï?${pid}`);
  }
...
function mapPrizeHeader(headers) {
  ...
  return {
    no:    find(['no','åºè?','?Ÿç¢¼','ç·¨è?','number']),
    id:    find(['id', 'ç·¨è?']),
    name:  find(['name', '?Žå?', '?Žé?', '?ç¨±', 'prize']),
    quota: find(['quota', '?é?', '?¸é?'])
  };
}
...
export async function drawBatch(n = 1, opts = {}) {
  try {
    ...
    if (!Array.isArray(people)) throw new Error('äººå“¡?å–®è®€?–å¤±??);
    if (!Array.isArray(prizes)) throw new Error('?Žé?è³‡æ?è®€?–å¤±??);

    const cur = prizes.find(p => p && p.id === curId);
    if (!cur) throw new Error('å°šæœª?¸æ??½ç??…ç›®');

    const need = prizeLeftLocal(cur);
    if (need <= 0) throw new Error('æ­¤ç??…å?é¡å·²æ»?);
    ...
    if (pool.length === 0) throw new Error('æ²’æ??¯æŠ½?å–®ï¼ˆè?æª¢æŸ¥?ºå¸­?€?‹æ?å·²æ?å¾—ç?ç´€?„ï?');
