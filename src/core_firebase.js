...
export async function listEvents(){const idx=await FB.get('/events_index')||{};return Object.entries(idx).map(([id,m])=>({id,name:m?.name||'ï¼ˆæœª?½å?ï¼?,client:m?.client||'',listed:m?.listed!==false})).sort((a,b)=>a.name.localeCompare(b.name));}
export async function createEvent(name,client=''){const id='e'+Math.floor(Date.now()+Math.random()*1e6).toString(36);const meta={name:name||'?°æ´»??,client,listed:true};await FB.patch(`/events/${id}/meta`,meta);await FB.put(`/events/${id}/info`,{title:meta.name,client,dateTime:'',venue:'',address:'',mapUrl:'',bus:'',train:'',parking:'',notes:''});await FB.put(`/events/${id}/people`,[]);await FB.put(`/events/${id}/prizes`,[]);await FB.put(`/events/${id}/currentPrizeId`,null);await FB.put(`/events/${id}/questions`,[]);await FB.put(`/events/${id}/photos`,[]);await FB.put(`/events/${id}/logo`,"");await FB.put(`/events/${id}/banner`,"");await FB.put(`/events_index/${id}`,meta);return id;}
