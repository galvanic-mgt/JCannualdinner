...
async function playResultsAnimation(poll){
  ...
  chart.innerHTML = '';
  status.textContent = '?≠ÊîæÁµêÊ??ïÁï´‰∏≠‚Ä?;
  ...
  for (let i = 0; i < fills.length; i++) {
    ...
    if (countEl) countEl.textContent = `${count} Á•®`;
  ...
  status.textContent = '?ïÁï´ÂÆåÁï¢?ÇÂ?Ê¨°È??äÂèØ?çÊí≠??;
}
