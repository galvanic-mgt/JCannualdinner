...
function cellHTML(w, idx, mode){
  ...
  const rerollBtn = (mode==='cms' || mode==='tablet') ? `<button class="btn small reroll" data-idx="${idx}">?çÊäΩ</button>` : '';
  return `
...
async function renderRerollLog(){
  ...
  let html = '<table><thead><tr><th>?ÇÈ?</th><th>?éÈ?</th><th>?üÂ?‰∏?/th><th>?πÁÇ∫</th></tr></thead><tbody>';
  for (const row of list) {
    const t = row.time ? new Date(row.time).toLocaleString() : '';
    const ori = row.replaced ? `${row.replaced.name||''}Ôº?{row.replaced.dept||''}Ôºâ` : '';
    const rep = row.replacement ? `${row.replacement.name||''}Ôº?{row.replacement.dept||''}Ôºâ` : '<span style="opacity:.6">??/span>';
  ...
  if (mode === 'cms') {
    ...
    const poll = async ()=>{
      try{
        ...
        // keep ?æÊ≠£?ΩÁ? + Ê≠§Á?Â∞öÈ? in sync with tablet selection
...
if (exportBtn) exportBtn.onclick = ()=> exportCurrentWinners();
  ...
  if (undoBtn) {
    undoBtn.onclick = async ()=>{
      const ok = confirm('Á¢∫Â?Ë¶ÅÂæ©?ü‰?‰∏ÄÊ¨°ÊäΩ?éÁ??úÔ?Ê≠§Â?‰ΩúÊ?ÁßªÈô§?õÊäΩ?∫Á?ÂæóÁ??Ö„Ä?);
      if (!ok) return;
      try{
        await undoLastDraw();
        // refresh HUD
